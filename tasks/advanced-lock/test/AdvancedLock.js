...
describe("AdvancedLock", function () {
  ...
  describe("lockForAuthorized", function () {
    it("Should release funds for the first authorized addresses", async function () {
    ...
    it("Should release funds for the second authorized addresses", async function () {
      const { advancedLock, lockInterval, lockedAmount, owner, otherAccount, thirdAccount } =
        await loadFixture(deployFixture);
      var array = [otherAccount.address, thirdAccount.address];
      await expect(advancedLock.lockForAuthorized(array, { value: lockedAmount }))
      ...
      await expect(advancedLock.connect(thirdAccount).withdraw(owner.address))
        .to
        .emit(advancedLock, "Withdrawal")
        .withArgs(thirdAccount.address, lockedAmount);
